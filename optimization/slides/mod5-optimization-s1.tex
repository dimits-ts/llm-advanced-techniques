   ...
   \end{frame}
	...
	\begin{frame}{A Framework for Estimating Costs}
		\begin{itemize}
			...
			\item Then, $\text{Total Cost} = \text{CPT}  N_{\text{tasks}}$
		\end{itemize}
		...
		\begin{columns}
			...
			\begin{column}{0.45\textwidth}
				\begin{itemize}
					...
					resulting in \alert{$ 8$ less} GPU memory and
					inference costs.
				\end{itemize}
			\end{column}
		\end{columns}
	...
	\begin{frame}{Downsides}
		\begin{columns}
			\begin{column}{0.5\textwidth}
				\begin{figure}
					\includegraphics[width=\columnwidth]{kv-cache-size.png}
					\caption{\url{https://arxiv.org/html/2606.19746v1}}
				\end{figure}
			\end{column}
			\begin{column}{0.5\textwidth}
				\begin{block}{KV Caching}
	...
	\begin{frame}{How Should I Comply?}
		\begin{block}{The Seven Principles}
			\begin{enumerate}
				\item \alert{Lawfulness, Fairness, and Transparency}
				
				\item \alert{Purpose Limitation:} Data must be collected for specified, explicit, and legitimate purposes.
				...
				\item \alert{Data Minimisation/Proportionality:} Data collected must be adequate, relevant, and strictly limited to what is necessary for the declared aim.
				...
				\item \alert{Accuracy:} Personal data must be accurate and, where necessary, kept up to date.
	...
	\begin{frame}{}
		\begin{block}{The Seven Principles}
			\begin{enumerate}
				\setcounter{enumi}{4}
				\item \alert{Storage Limitation:} Data must only be kept in a form that permits identification for no longer than is necessary for the processing purpose.
				...
				\item \alert{Integrity and Confidentiality (Security):} Data must be protected by appropriate technical and organizational measures against unauthorized access, loss, or damage.
				...
				\item \alert{Accountability:} Controllers must actively implement and continuously demonstrate compliance with all data protection provisions in their activities.
